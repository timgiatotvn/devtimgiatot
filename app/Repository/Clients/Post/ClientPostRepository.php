...
class ClientPostRepository implements ClientPostRepositoryInterface
{
    ...
    {
        return DB::table(self::TABLE_NAME)
            ->select('posts.*', 'admins.name as author_name', 'categories.title as category_title', 'categories.slug as category_slug', 'categories.parent_id as category_parent_id', 'users.name as customer_name')
            ->leftJoin('categories', 'posts.category_id', 'categories.id')
