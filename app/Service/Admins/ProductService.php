...
class ProductService
{
    ...
    public function create($_data)
    {
        try {
            ...
            if (!empty($_data['keyword_suggest'])) {
            ...
            $db = array_merge($_data, [
                ...
                'type' => !empty($_data["type"]) ? $_data["type"] : (!empty($result) ? $this::TYPE[1] : $this::TYPE[0]),
                'keyword_suggest_map_crawler' => !empty($result) ? $result['data'] : NULL,
                'website_map' => !empty($result) ? $result['website_map'] : NULL,
                'price' => !empty($result['price_min']) ? $result['price_min'] : (!empty($_data['price']) ? $_data['price'] : "0"),
                'price_root' => !empty($result['price_min']) ? $result['price_min'] : (!empty($_data['price_root']) ? $_data['price_root'] : "0"),
                'count_suggest' => !empty($result) ? $result['total'] : 0,
                'thumbnail_cr' => !empty($result) ? $result['thumbnail'] : (!empty($_data['thumbnail']) ? $_data['thumbnail'] : ""),
                'thumbnail' => !empty($result) ? $result['thumbnail'] : (!empty($_data['thumbnail']) ? $_data['thumbnail'] : ""),
                'choose_1' => 0,
                'choose_2' => 0,
                'choose_3' => 0,
                'choose_4' => 0,
