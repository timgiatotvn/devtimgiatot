                            ...
                            <table class="table table-striped table-bordered table-hover">
                                ...
                                @foreach($data['list'] as $k=>$row)
                                    <tr>
                                        ...
                                        <td>
                                            <a class="icon-form status active" href="{{ route('admin.category.status', ['id' => $row->id, 'field' => 'choose_2']) }}">
                                                {!! (($row->choose_2 == 1) ? '<i class="icon-check"></i>' : '<i class="icon-close"></i>') !!}
                                            </a>
                                        </td>
                                        <td>
                                            <a class="icon-form status active" href="{{ route('admin.category.status', ['id' => $row->id, 'field' => 'choose_3']) }}">
                                        ...
                                        <td>{{ $row->id }}</td>
                                        <td>
                                            <a class="icon-form" title="edit" href="{{ route('admin.category.edit', ['id' => $row->id, 'page' => $data['list']->currentPage(), 'parent_id' => (request()->has('parent_id') ? request()->get('parent_id') : '')]) }}">
                                                <i class="icon-note"></i>
                                            </a>
                                            <a class="icon-form status active" href="{{ route('admin.category.status', ['id' => $row->id, 'field' => 'status']) }}">
                                                {!! (($row->status == 1) ? '<i class="icon-check"></i>' : '<i class="icon-close"></i>') !!}
                                            </a>
                                            <a class="icon-form" href="javascript:confirmDelete('{{ route('admin.category.destroy', ['id' => $row->id]) }}','@lang('admins::layer.notify.confirm.delete')')">
